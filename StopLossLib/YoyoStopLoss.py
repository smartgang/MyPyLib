...
def get_long_stop_loss_by_tick(bar_df):
    bardf = pd.DataFrame({'high': bar_df['longHigh'], 'low': bar_df['longLow'], 'strtime': bar_df['strtime'], 'utc_time': bar_df['utc_time'],
                          'last_close': bar_df['last_close'], 'yoyo_value': bar_df['yoyo_value']})
    # 多仓止损
...
def yoyo(strategyName, symbolInfo, K_MIN, setname, bar1mdic, barxmdic, sl_para_dic, result_para_dic, tofolder, indexcols, timestart):
    ...
    # 保存新的result文档
    oprdf.to_csv(tofolder + strategyName + ' ' + symbol + str(K_MIN) + ' ' + setname + ' result_yoyo_by_tick.csv', index=False)
    ...
    dR.calDailyResult()
    dR.dailyClose.to_csv((tofolder + strategyName + ' ' + symbol + str(K_MIN) + ' ' + setname + ' dailyresult_yoyo_by_tick.csv'))
